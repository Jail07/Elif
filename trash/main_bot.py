...
from admin.main_admin import insert_project
...
def CALLBACK_C_message2(message, qwerty):
    ...
    bot.send_message(chat_id, 'What is the name of your project?')
    bot.register_next_step_handler(message, project_description)

    # else:
    #     bot.send_message(chat_id, 'Unknown option')

def project_description(message):
    global name_project
    name_project = message.text.strip()
    bot.send_message(message.chat.id, 'What is your project about?')
    bot.register_next_step_handler(message, deadline())
...
def finalize_order(message, department):
    ...
    insert_project(message, user_name, user_id, name_project, project_description, your_date)
...
#     if CHECK_ADMIN:
#         insert_project_main(message, project_name, about, department, deadline)
...
# def insert_project_main(message, project_name, about, department, deadline):
#     try:
#         conn = create_conn('students.sql')
#         cur = conn.cursor()
...
def CALLBACK_C_message(callback, conn):
    ...
    if callback.data == CALLBACK_LIST_PROJECT:
        if chek_st(callback.message):
